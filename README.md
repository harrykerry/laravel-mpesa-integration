...
Email: haroldkerry@gmail.com | +254 743417198
Website: haroldkerry.co.ke | volcha.co.ke
...
-   **Controllers**: Manage incoming requests and responses.
    ...
    -   `MpesaDataFetchController`: Fetches stored M-PESA transactions.
    -   `mpesaStkController`: Handles STK requests and callback.
...
-   **Services**: Encapsulate business logic.
    ...
    -   `MpesaDataFetchService`: Fetches transcations stored in the database.
    -   `MpesaStkService`: Initiates STK Push requests and handles incoming stk data.
...
-   **Routes**: Define the API endpoints.
    -   `POST /payments/c2b/confirmation/callback`: Endpoint for M-PESA confirmation callback.
...
-   **M-PESA C2B Confirmation Callback**:
    ...
    -   The `/payments/c2b/confirmation/callbackk` endpoint will receive callback data from M-PESA and store it in the database.

-   **M-PESA C2B Validation Callback**:

    -   The `/payments/c2b/validation/callback` endpoint perfoms custom validation and responds to M-PESA with a JSON response to accept or reject transactions.
...
-   **Initiate STK Request**:

    -   The `/mpesa/stk/initiate` This endpoint initiates the STK push (lipa na M-Pesa) request to M-PESA

-   **STK Callback**:

    -   The `/mpesa/stk/callback` This endpoint handles the callback response from M-PESA after an STK push has been processed.

-   **Fetch Stored Transactions**:

    -   The `/mpesa/payments/c2b` `/mpesa/payments/stk` endpoints allows external systems to fetch stored transactions.
